...
@section('content')
    ...
    <div class="modal fade" id="createRuanganModal" tabindex="-1" role="dialog" aria-labelledby="createRuanganModalLabel"
        aria-hidden="true">
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#createRuanganForm').append(
                            '<input type="hidden" name="_method" value="PUT">');
                        $('#createRuanganForm').attr('action',
                            "{{ route('master.ruangan.update', ':id') }}".replace(':id', id)
                            );
            ...
            $(document).on('click', '.btn-delete', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('master.ruangan.destroy', ':id') }}".replace(
                                ':id', id),
                            dataType: "json",
                            headers: {
